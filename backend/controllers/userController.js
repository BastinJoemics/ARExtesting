...
const registerUserWithDevices = asyncHandler(async (req, res) => {
    const { firstName, lastName, email, password, deviceIds } = req.body;
  
    // Validation
    if (!firstName || !lastName || !email || !password || !deviceIds || deviceIds.length === 0) {
      res.status(400);
      throw new Error("Please fill in all required fields and select at least one device");
    }
    if (password.length < 6) {
      res.status(400);
...
const getUserDevices = asyncHandler(async (req, res) => {
    try {
        ...
        res.status(200).json(user.assignedDevices);
    } catch (error) {
